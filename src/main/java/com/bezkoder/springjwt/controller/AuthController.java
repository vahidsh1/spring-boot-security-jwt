...
import com.bezkoder.springjwt.service.AuthenticationService;
...
public class AuthController extends BaseController {
    ...
    public ResponseEntity<ApiResponse<Object>> authenticateUser(@RequestBody LoginRequest loginRequest) {
        // Delegate authentication to the AuthenticationService
        ...
        // Assuming `isFirstLogin` is a field in `UserEntity`, modify this if needed
        boolean isFirstLogin = userEntity.isFirstLogin();

        JwtResponse jwtResponse = new JwtResponse(jwt, userEntity.getId(), userDetails.getUsername(),
                userEntity.getEmail(), isFirstLogin, roles);

        return sendSuccessResponse(HttpStatus.OK, jwtResponse, ResponseCode.SUCCESS); // Updated to use ResponseCode
    }

    @PostMapping("/signup")
    public ResponseEntity<ApiResponse<Object>> registerUser(@RequestBody SignupRequest signUpRequest) {
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
            return sendErrorResponse(HttpStatus.BAD_REQUEST, ResponseCode.USERNAME_ALREADY_EXISTS); // Updated to use ResponseCode
        }

        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            return sendErrorResponse(HttpStatus.BAD_REQUEST, ResponseCode.EMAIL_ALREADY_EXISTS); // Updated to use ResponseCode
        }
        ...
        UserEntity userEntity = new UserEntity(signUpRequest.getUsername(), signUpRequest.getEmail(), encoder.encode(signUpRequest.getPassword()),
                true, roles);

        userRepository.save(userEntity);
        return sendSuccessResponse(HttpStatus.CREATED, userEntity, ResponseCode.USER_REGISTERED_SUCCESSFULLY); // Updated to use ResponseCode
    }

    @PostMapping("/change-password")
    public ResponseEntity<ApiResponse<Object>> changePassword(@RequestBody ChangePasswordRequest changePasswordRequest) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();

        if (changePasswordRequest.getOldPassword().equals(changePasswordRequest.getNewPassword())) {
            return sendErrorResponse(HttpStatus.BAD_REQUEST, ResponseCode.PASSWORD_SAME_AS_OLD); // Updated to use ResponseCode
        }
        ...
        if (!encoder.matches(changePasswordRequest.getOldPassword(), user.getPassword())) {
            return sendErrorResponse(HttpStatus.BAD_REQUEST, ResponseCode.INCORRECT_CURRENT_PASSWORD); // Updated to use ResponseCode
        }

        user.setPassword(encoder.encode(changePasswordRequest.getNewPassword()));
        userRepository.save(user);
        return sendSuccessResponse(HttpStatus.OK, null, ResponseCode.PASSWORD_CHANGED_SUCCESSFULLY); // Updated to use ResponseCode
    }
}
